...
import { useImportPatientMutation } from "@/services/queries/patient";

export default function ImportPatientBtn() {
  const [file, setFile] = useState(null);
  const [isOpen, setIsOpen] = useState(false);
  const [isSuccess, setIsSuccess] = useState(false);

  const { mutate, isPending } = useImportPatientMutation();
        ...
        {isSuccess ? (
          <div className="flex py-4 items-center justify-center text-center  text-green-600">
            <div className="space-y-2">
              ...
              <p className="font-medium">Patients imported successfully!</p>
            </div>
          </div>
        ) : (
          <>
